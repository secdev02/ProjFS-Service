 ...
 *   displays toast notifications when virtual files are accessed.
 * 
 * Dependencies:
 ...
 *   - Windows.Data.dll and Windows.UI.dll (for toast notifications)
 * 
 * 
 * Compilation:
 *   csc ProjFS-Service.cs /reference:"C:\Program Files (x86)\Windows Kits\10\UnionMetadata\10.0.17763.0\Windows.winmd" /reference:"C:\Program Files (x86)\Reference Assemblies\Microsoft\Framework\.NETCore\v4.5\System.Runtime.WindowsRuntime.dll"
 * 
 * Installation:
 ...
 *   - Toast notifications show which file was accessed and by which process
 *   - Ensure Windows notifications are enabled for toast alerts to appear
 * 
 * License: MIT License
 *
 * 
...
\Network\Router Configuration.xml,false,25267,1741508986";
    ...
    class ProjFSProvider
    {
        ...
        {
            Console.WriteLine(string.Format("File accessed: {0} from process {1}", filePath, imgFileName));
            ...
            try
            {
                    ...
                    @"<toast>
                        <visual>
                            <binding template='ToastGeneric'>
                                <text>File Access Detected</text>
                                <text>File: {0}</text>
                                <text>Process: {1}</text>
                            </binding>
                        </visual>
                ...
                ToastNotificationManager.CreateToastNotifier("WindowsFakeFileSystem").Show(toast);
            }
            catch (Exception ex)
            {
